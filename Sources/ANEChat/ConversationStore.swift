...
class ConversationStore: ObservableObject {
    ...
    func updateMessageContent(_ messageId: UUID, content: String) {
        guard let conversationIndex = conversations.firstIndex(where: { $0.id == selectedConversationId }) else { return }
    ...
    private func saveConversations() {
        ...
        if let selectedId = selectedConversationId {
            UserDefaults.standard.set(selectedId.uuidString, forKey: selectedConversationKey)
        } else {
            UserDefaults.standard.removeObject(forKey: selectedConversationKey)
        }
    }
    ...
    private func loadConversations() {
        if let data = UserDefaults.standard.data(forKey: saveKey),
           let decoded = try? JSONDecoder().decode([Conversation].self, from: data) {
            ...
            if let selectedIdString = UserDefaults.standard.string(forKey: selectedConversationKey),
